...
# ייבוא פונקציית יצירת הפונט אם קיימת
try:
...
# מיפוי הזזות אנכיות מותאמות לפי אותיות (פיקסלים)
vertical_offsets = {
...
def process_image_to_bw_bytes(image_bytes):
    """מעבד תמונה גלובלית, מחזיר תמונה בשחור-לבן ב־PNG bytes"""
...
def normalize_and_center_glyph(img, target_size=600, margin=50, vertical_offset=0):
    ...
    if coords is None:
        # תמונה ריקה לבנה אם לא נמצאו פיקסלים
...
@app.route('/')
def index():
    # דף ראשי עם העלאת תמונה
    return render_template('index.html')

@app.route('/api/upload', methods=['POST'])
def api_upload():
    # מקבל קובץ תמונה, שומר ומחזיר תמונה מעובדת בשחור-לבן ב-base64
...
@app.route('/crop')
def crop_page():
    # דף חיתוך שבו המשתמש יחתוך אותיות מתוך התמונה המעובדת
...
def api_save_crop():
    # מקבל חיתוך אותית בשחור לבן ב-base64 ושם+אינדקס ושומר
...
def api_list_glyphs():
    # מחזיר רשימת הקבצים שנשמרו בתיקיית אותיות
    files = sorted([p.name for p in GLYPHS.glob("*.png")])
    return jsonify({"files": files})

@app.route('/api/finalize', methods=['POST'])
def api_finalize():
    # מפעיל את יצירת הפונט מהתמונות שנשמרו
    data = request.get_json()
